...
class oxSeoEncoder extends oxSuperCfg
{
     ...
     * @param bool   $blForceSubDomain Force subdomain.
     * @param bool   $blForceDomain    Force domain
     *
     * @return string
     */
    protected function _getFullUrl($sSeoUrl, $iLang = null, $blSsl = false, $blForceSubDomain = false, $blForceDomain = false)
    {
        /** @var oxConfig|ShopInn_oxConfig $oConfig */
        $oConfig = $this->getConfig();

        if ($sSeoUrl) {
            $sShopUrl = ($blForceSubDomain) ? $oConfig->getShopSubDomainUrl()
                : ($blForceDomain ? $oConfig->getShopDomainUrl() : $oConfig->getShopUrl($iLang, false));
            $sFullUrl = ($blSsl ? $oConfig->getSslShopUrl($iLang) : $sShopUrl) . $sSeoUrl;
     ...
     * @param bool   $blForceSubDomain Force subdomain use.
     *
     * @return string
     */
    public function getStaticUrl($sStdUrl, $iLang = null, $iShopId = null, $blForceSubDomain = false)
    {
        if (!isset($iShopId)) {
            $iShopId = $this->getConfig()->getShopId();
        }
        ...
        if (($sSeoUrl = $this->_getStaticUri($sStdUrl, $iShopId, $iLang))) {
            $sFullUrl = $this->_getFullUrl($sSeoUrl, $iLang, strpos($sStdUrl, "https:") === 0, $blForceSubDomain);
        }
