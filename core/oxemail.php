...
class oxEmail extends PHPMailer
{
    ...
    */
    ...
    protected function _sendMailErrorMsg()
    {
        ...
        //$header = "From: no-reply@shopinn.lt <no-reply@shopinn.lt>\r\n";
        ...
        if(!empty($sShopDomainUrl) && $bSPF)
            $header = "From: no-reply@" . $sShopDomainUrl . " <no-reply@" . $sShopDomainUrl . ">\r\n";
        else
            $header = "From: no-reply@" . $sShopURL . " <no-reply@" . $sShopURL . ">\r\n";

        $blRet  = @mail( $oShop->oxshops__oxorderemail->value, "eMail problem in shop!", $sOwnerMessage, $header);
