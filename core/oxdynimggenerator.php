...
class oxDynImgGenerator
{
    ...
    protected function _generateImage( $sImageSource, $sImageTarget )
    {
        ...
        if ( getGdVersion() !== false && $this->_isTargetPathValid( $sImageTarget ) && ( $sImageType = $this->_getImageType() ) ) {
            ...
            // watermark start
            //if( oxRegistry::getConfig()->getConfigParam("blUseWatermarkOnSmall") ) {

                preg_match('/pictures.generated.product.(?:\d*\.)?\d+.490_490/', $sImageTarget, $matches);

                if($matches) {

                    preg_match('/product.(?:\d*\.)?\d+/',$sImageSource, $nummatches);
                ...
                preg_match('/pictures.generated.product.(?:\d*\.)?\d+.800_800/', $sImageTarget, $matches);
    ...
    public function getImagePath( $sAbsPath = false )
    {
        ...
        if ( file_exists( $sMasterImagePath ) ) {
            ...
            // Watermark start
            preg_match('/pictures.master.product/', $sMasterPath . $this->_getImageName(), $matches);
            if($matches) {
                $sWMPath = str_replace('/master/', '/generated/', $sMasterPath) . "wm";
                if( !file_exists( $sWMPath ) )
                    mkdir( $sWMPath, 0777, true ); //Create directory

                //Add Watermark and move to destination
                $oxUtilsFile = oxNew('oxUtilsFile');
                $oxUtilsFile->addWatermark($sMasterPath.$this->_getImageName(), $sWMPath, $this->_getImageName());
            }
            // Watermark end
        } else {
            // nopic master path
