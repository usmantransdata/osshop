...
class oxViewConfig extends oxSuperCfg
{
     ...
     * @return string
     */
    public function getPriceEur($sPrice)
    {
        ...
        if (!empty($sPrice)) {
            ...
            $sPrice = @str_replace($sThousandSeparator, "", trim($sPrice, chr(0xC2) . chr(0xA0))); //Thousands separator remove
            $sPrice = @str_replace($sDecimalSeparator, ".", $sPrice); //Decimal separator change
            $dPrice = floatval($sPrice);

            if (!empty($dPrice)) {
                //$dPrice = $dPrice / $dEurVaulue;
                $dPrice = $dPrice * $dEurVaulue;
                $dPrice = round($dPrice, 2, PHP_ROUND_HALF_UP);

                return oxRegistry::getLang()->formatCurrency($dPrice) . ' Lt';
            } else {
                return oxLang::getInstance()->formatCurrency($dPrice) . ' Lt';
            }
        }
    ...
    {
        $iLangId = oxRegistry::getLang()->getTplLanguage();

        return oxRegistry::getLang()->getLanguageAbbr($iLangId);
    }

    public function getTplLanguageId()
    {
