...
function smarty_function_oxinputhelp($params, &$smarty)
{
    ...
    if ((!$sTranslation || $sTranslation == $sIdent) && !$sImg) {
    ...
    if ($sImg) {
        $sImgPath = oxRegistry::getConfig()->getImageUrl(false, null, false, $sImg);
        $smarty->assign('sImagePath', $sImgPath);
    } else {
        $smarty->assign('sImagePath', null);
    }
